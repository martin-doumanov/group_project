...
In our final group assignment, we will analyse data of Airbnb listings and fit a model to predict the total cost for two people staying 4 nights in an AirBnB in Berlin. We will download AirBnB data from [insideairbnb.com](http://insideairbnb.com/get-the-data.html){target="_blank"}; it was originally scraped from airbnb.com. 
...
# We use cache=TRUE so we dont download the data every time we knit
...
There are many variables in the dataframe. Here is a quick description of some of the variables collected, and you can find a [data dictionary here](https://docs.google.com/spreadsheets/d/1iWCNJcSutYqpULSQHlNyGInUvHg2BoUGoNRIGa6Szc4/edit#gid=982310896)
...
- `room_type`:
  ...
  - Hotel room (Hotel rooms offered through AirBnB)

- `number_of_reviews`: Total number of reviews for the listing
- `review_scores_rating`: Average review score (0.0 - 5.0)
...
Our dataset has many more variables, so we attempt to extend our analysis by adding a number of variables, which could prove useful in increasing the explanatory power of our model. As a start, we will address the following questions:
...
Next step is to investigate the explanatory power of host_is_superhost as an independent variable.
...
Now, we check the residuals of our models. Furthermore, we compare all of our models' statistical significance and explanatory power using the huxtable package
...
Comparing the statistical significance and explanatory power of all of our models, we maintain our view that model9 is the superior model. This is given its overall statistical significance as well as variable-specific statistical significance. Furthermore, we find that the explanatory power of the model is superior given the relatively low number of explanatory variables compared to other models with similar explanatory power such as model8, where a large number of explanatory variables may increase risk of overfitting. Finally, we find that, intuitively, all our variables in model9 seem rationally correlated with the price of an AirBnB listing. Thus, we we will use model9 in our prediction and analysis below.
...
```
Based on our predictions, the average price for 4 nights is expected to be approximately *163 dollars*, with a 95% confidence interval ranging from 158.79-166.34.
