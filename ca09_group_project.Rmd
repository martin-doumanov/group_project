...
Next we are going to create a new variable price_4_nights, which represents the price we would have to pay to stay at an Airbnb property for 4 nights. Since we are interested in renting for two, we will exclude any properties that accommodate less than 2 people. We also exclude any properties with negative or 0 prices.
...
model1 %>% 
   broom::glance()
```
Overall, Model 1 has p-value of 0, which means it is statistically significant. Furthermore, according to R squared, our model explains 34% of the variation in log_price_4_nights with an adjusted R squared of 34%. Additionally, all the variables used are significant at the 95% confidence level. 

Since the variable property_type is categorical  with 5 possible options, the model only shows 4 coefficients. The fifth one (Entire Condo), is incorporated in the intercept. This means the coefficients of the the other four are in relation to the fifth one. To interpret the coefficients for a log-transformed dependent variable, we are going to take the exponent of the coefficient. In this case, entire rental units are on average 12.0% cheaper than entire condos for 4 nights. Similarly, entire serviced apartments are 67.4% more expensive than entire condos, whereas privet rooms in rental units are 51.1% cheaper than entire condos. Finally, other types of properties are on average 17.5% cheaper than entire condos.
...
```
Model 2 is also statistically significant, with a p-value of 0. Additionally, it explains 39.23% of the vriation in log_price_4_nights, which is higher than Model 1. All variables are statistically significant. To compare the two models, however, we need to look at the adjusted r-squared, since Model 2 includes one additional variable. Model 2 has an adjusted r-squared of 39.16% compared to 34.02% for Model 1. We can conclude that Model 2 has a higher predictive power than Model 1.

Since room_type is a categorical variable with 4 possible options, the model only shows 3 coefficients. The fourth option (Entire Home/ Apartment) is incorporated in the intercept. To interpret the coefficients for a log-transformed dependent variable, we are going to take the exponent of the coefficient. This means that hotel rooms, for example, are on average 38.9% more expensive than entire home/apartments. Similarly, private rooms 50.7% cheaper, whereas shared rooms are 66.9% cheaper than entire homes.
...
car::vif(model2)
```
We  can see that room_type and property_type_simplified have VIF socres of more than 10, which means there is potential multicollinearity problem in our model. Before we proceed with the analysis, we will test which variable has a higher predictive power and remove the other:
...
We see from the table that Model 3 is statistically significant and adding information about property amenities like number of bedrooms and how many people a property can accommodate greatly increases the predictive power of the model. We see a r-squared of 52.4% and an adjusted r-squared of 52.3%. However, when we look at the coefficients, we see that both number of beds and number of reviews are not statistically significant. This is surprising, since number of reviews was significant in our previous model specifications.

One potential explanation for this could be that there is multicollinearity. If we think about it, the number of people a property will accomodate should be closely related to the number of beds in the property, as well as to the number of bedrooms. We check this with a VIF test below:
...
# Model 4 - Model 3 without `accommodates`

model4 <- lm(log_price_4_nights ~ number_of_reviews + 
                                  review_scores_rating +
                                  room_type +
                                  bedrooms +
                                  beds,
...
# We construct a table with the regression output
huxtable:: huxreg(list("No-Accommodates Model" = model4), 
                  statistics = c('#observations' = 'nobs', 
                                'R squared' = 'r.squared', 
                                'Adj. R Squared' = 'adj.r.squared', 
                                'Residual SE' = 'sigma',
...
We can see that now, both beds and number of reviews are statistically significant. The predictive power of our model has not decreased by much either (50.1% compared to 52.3% adjusted r-squared). We will proceed with Model 4.
...
```
Model 5 is statistically significant, with a p-value of 0. Additionally, Model 5 has a slightly better predictive power than Model 4, although the differences in adj. r-squared are very small (0.1%). The new variable we added, host_is_superhost, is statistically significant, but now the number_of_reviews is not. One potential explanation is that the the number of reviews has different effect on the price depending on if the host is a superhost or not. To deal with this, we will create an interaction term between `host_is_superhost` and `number_of_reviews`:
