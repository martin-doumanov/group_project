...
listings_train %>% #skimming only the training data, not test to avoid cheating
  skim()
...
listings_train %>% 
  ...
  select(skim_variable)
```
Notice that the variables 'host_response_rate', 'host_acceptance_rate' and 'price' have been categorised as character strings instead of numeric variables.
...
typeof(listings_train$price)
typeof(listings_train$host_response_rate)
typeof(listings_train$host_acceptance_rate) #Price, host response rate and host acceptance rate are now stored as numbers for the training set
...
typeof(listings_test$price)
typeof(listings_test$host_response_rate)
typeof(listings_test$host_acceptance_rate) #Price, host response rate and host acceptance rate are now stored as numbers for the test set
...
Now let's look at the price data in more detail. We will start by looking at the correlations between price and the other numeric variables.

#Add correlation data
#correlation data not linear so take logs
#first check that there are no negative/zero values



```{r}

favstats(~price, data = listings_train)
...
```
There are 6 observations where price takes the value 0.
...
BUILD A VARIABLE PRICE PER 4 NIGHTS. THIS IS THE DEPENDENT VARIABLE WHICH WE NEED TO MODEL.
